# h2 DORA the Explora

## x) 

### Buuri 2026: DORA and TLPT testing - Lecture for Haaga-Helia on 31 March 2026 (pdf, 2 MB)

- TIBER on framework red team testaukseen
- Hyökkäyksissä voidaan antaa legup, jolloin red teamille annetaan esim. jokin tunnus jolla ohittaa vaihe, jossa voisi jäädä kiinni.

### DORA (Regulation ... on digital operational resilience for the financial sector)

- TLPT-testauksessa jäljitellään oikeaa kyberhyökkystä.
- Testaajat ovat osaavia ja luotettavia.

### TIBER-FI procedures and guidelines 

- On ennalta määritellyt kohteet, joita halutaan testata.
- Red Team jäljittelee oikeaa hyökkääjää.

### 

## a) Asenna Metasploitable 2 virtuaalikoneeseen.

![Metasploitableasennus](images/metasploitable1.png)

- Ram: 1024mb
- cpu: 1

## b)  Tee Kalin ja Metasploitablen välille virtuaaliverkko.

![Metasploitableasennus](images/metasploitable2.png)
- Kali adapter 1
![Metasploitableasennus](images/metasploitable3.png)
- kali adapter 2
![Metasploitableasennus](images/metasploitable4.png)
- metasploitable adapteri

## c)  

![Metasploitableasennus](images/metasploitable5.png)
- Metasploitablen ifconfig

![Metasploitableasennus](images/metasploitable6.png)
- Kali ei pääse nettiin, mutta yhteys metasploitableen onnistuu

## d) 

![Metasploitableasennus](images/metasploitable7.png)
- nmap löysi metasploitable ip:n
![Metasploitableasennus](images/metasploitable8.png)
- Sivu avautui selaimessa

## e) 

![metasploitable](images/metasploitable9.png)
- nmap skannaus valmis
![metasploitable](images/metasploitable10.png)
- tuloksia nmap skannauksesta
- Portti 139 ja 445: SMB --> Tiedostojen jako palvelu.
- Portti 3306: MySQL --> Tietokantapalvelu. Voi sisältää tärkeää tietoa.
- Portti 512: rexec --> Vanha etäkomentopalvelu. Sen avulla voi suorittaa komentoja.
- Kaikissa näissä vanhat versiot ja niiden huonosuojaus voi olla riski. 









